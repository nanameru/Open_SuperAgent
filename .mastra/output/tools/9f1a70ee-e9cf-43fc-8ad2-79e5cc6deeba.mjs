...
const CustomScriptSchema = z.object({
  ...
  timeout: z.number().optional().default(3e4).describe("\u30B9\u30AF\u30EA\u30D7\u30C8\u5B9F\u884C\u306E\u30BF\u30A4\u30E0\u30A2\u30A6\u30C8\uFF08\u30DF\u30EA\u79D2\uFF09")
});
...
const browserbaseToolInputSchema = z.object({
  ...
  waitTime: z.number().min(1e3).max(12e4).optional().default(3e3).describe("\u5F85\u6A5F\u6642\u9593\uFF08\u30DF\u30EA\u79D2\u3001wait\u30A2\u30AF\u30B7\u30E7\u30F3\u3067\u4F7F\u7528\uFF09"),
  ...
  // CAPTCHA解決用オプション
  captchaTimeout: z.number().optional().default(6e4).describe("CAPTCHA\u89E3\u6C7A\u306E\u30BF\u30A4\u30E0\u30A2\u30A6\u30C8\uFF08\u30DF\u30EA\u79D2\uFF09"),
...
async function executeAction(page, action, options) {
  ...
  let screenshotInfo;
  let elementText;
  let scriptResult = void 0;
  switch (action) {
    case "navigate":
      break;
    case "screenshot":
      const screenshotName = `browserbase_${v4()}.png`;
      const screenshotPath = path__default.join(imagesDir, screenshotName);
    ...
    case "get_text":
      ...
      elementText = await page.textContent(selector) || void 0;
  ...
  if (!["screenshot", "get_title", "get_text", "wait"].includes(action)) {
    const autoScreenshotName = `browserbase_auto_${v4()}.png`;
    const autoScreenshotPath = path__default.join(imagesDir, autoScreenshotName);
...
function generateMarkdownContent(params) {
  ...
  let markdownContent = `## \u{1F310} \u30D6\u30E9\u30A6\u30B6\u81EA\u52D5\u5316\u5B8C\u4E86

`;
  markdownContent += `**\u{1F4CB} \u5B9F\u884C\u7D50\u679C:**
`;
...
`;
  if (liveViewUrl) {
    markdownContent += `\u{1F534} **\u30E9\u30A4\u30D6\u30D3\u30E5\u30FC:** [\u30EA\u30A2\u30EB\u30BF\u30A4\u30E0\u8868\u793A](${liveViewUrl})
`;
  }
  markdownContent += `\u{1F3AC} **\u30BB\u30C3\u30B7\u30E7\u30F3\u30EA\u30D7\u30EC\u30A4:** [\u64CD\u4F5C\u8A18\u9332\u3092\u8868\u793A](${replayUrl})

`;
  if (screenshotInfo) {
    markdownContent += `\u{1F4F8} **\u30B9\u30AF\u30EA\u30FC\u30F3\u30B7\u30E7\u30C3\u30C8:** \u64CD\u4F5C\u5F8C\u306E\u753B\u9762\u3092\u30AD\u30E3\u30D7\u30C1\u30E3\u3057\u307E\u3057\u305F

`;
  }
  if (action === "solve_captcha" || captchaSolved) {
    markdownContent += `\u{1F510} **CAPTCHA\u89E3\u6C7A\u6A5F\u80FD:**
`;
...
`;
  markdownContent += `*\u30BB\u30C3\u30B7\u30E7\u30F3: ${session.id} | \u5B9F\u884C\u6642\u523B: ${(/* @__PURE__ */ new Date()).toLocaleString("ja-JP")}*`;
  return markdownContent;
}
const browserbaseTool = createTool({
  id: "browserbase-automation",
  description: "Browserbase\u3092\u4F7F\u7528\u3057\u3066\u30AF\u30E9\u30A6\u30C9\u4E0A\u3067\u30D6\u30E9\u30A6\u30B6\u3092\u81EA\u52D5\u5316\u3057\u307E\u3059\u3002Web\u30DA\u30FC\u30B8\u306E\u8A2A\u554F\u3001\u30B9\u30AF\u30EA\u30FC\u30F3\u30B7\u30E7\u30C3\u30C8\u53D6\u5F97\u3001\u8981\u7D20\u64CD\u4F5C\u3001\u30C6\u30AD\u30B9\u30C8\u5165\u529B\u3001CAPTCHA\u89E3\u6C7A\u3001\u30AB\u30B9\u30BF\u30E0\u30B9\u30AF\u30EA\u30D7\u30C8\u5B9F\u884C\u306A\u3069\u304C\u53EF\u80FD\u3067\u3059\u3002\u30BB\u30C3\u30B7\u30E7\u30F3\u30EA\u30D7\u30EC\u30A4\u6A5F\u80FD\u306B\u3088\u308A\u64CD\u4F5C\u5185\u5BB9\u3092\u5F8C\u304B\u3089\u78BA\u8A8D\u3067\u304D\u307E\u3059\u3002",
...
export { advancedCalculatorTool, braveSearchTool, browserbaseTool, geminiImageGenerationTool, geminiVideoGenerationTool, graphicRecordingTool, grokXSearchTool, htmlSlideTool, imagen4GenerationTool, minimaxTTSTool, presentationPreviewTool, v0CodeGenerationTool, weatherTool };
